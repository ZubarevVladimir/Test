...
public class Browser {
  ...
  private final int WAIT_ELEMENT_TIMEOUT = 10;
  ...
  public static RemoteWebDriver getAutoitDriver() {
    if (autoitDriver == null) {
      DesiredCapabilities autoitCapabilities = new DesiredCapabilities();
      String hostName = "10.6.132.40";
      autoitCapabilities.setCapability("browserName", "AutoIt");

      try {
        autoitDriver = new RemoteWebDriver(new URL("http://" + hostName + ":4723/wd/hub"),
            autoitCapabilities);
  ...
  private static Browser init() {
    ...
    try {
      ...
      System.out.println("Timeout exception" + ex.getLocalizedMessage());
    }
  ...
  public static void kill() {
    if (instance != null) {
      try {
        ...
        System.err.println("Can not kill browser " + e.getLocalizedMessage());
      } finally {
        instance = null;
      }
    }
